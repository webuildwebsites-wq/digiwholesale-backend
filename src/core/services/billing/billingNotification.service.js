...
import {
  ...
  generatedorderInvoice,
} from "../../../Utils/templates/deliveryChallanTemplate.js";
import { sendEmail } from "../../config/Email/emailService.js";
import {
  sendWhatsAppOTP,
  sendWhatsAppMedia,
} from "../../config/Whatsapp/sendWhatsappOtp.js";
...
const buildChallanData = (bulkOrder, customer) => ({
  ...
  companyAddress:
    process.env.COMPANY_ADDRESS || "WeWork Eldeco Centre, New Delhi",
  ...
  customerAddress:
    bulkOrder.customer.customerShipToBranchName ||
...
const buildInvoiceData = (bulkOrder, customer) => {
  ...
  const shipToAddress = bulkOrder.customer.customerShipToId
    ? customer?.customerShipToDetails?.find(
        (s) =>
          s._id.toString() === bulkOrder.customer.customerShipToId.toString(),
      )
    : null;
  ...
  const allItems = bulkOrder.orders.flatMap((order) =>
    order.items.map((item) => {
      ...
      return {
        ...
        dcNo: "",
        orderDate: new Date(
          order.createdAt || bulkOrder.createdAt,
  ...
  const discountAmount = allItems.reduce(
    (s, i) => s + (Number(i.value) - Number(i.netValue)),
    0,
  );
  const taxableAmount = grossAmount - discountAmount;
  const cgstAmount = bulkOrder.orders.reduce(
    (s, o) => s + (parseFloat(o.cgst) || 0),
    0,
  );
  const sgstAmount = bulkOrder.orders.reduce(
    (s, o) => s + (parseFloat(o.sgst) || 0),
    0,
  );
  ...
  return {
    ...
    shipTo: {
      name:
        bulkOrder.customer.customerShipToBranchName ||
        bulkOrder.customer.customerName,
...
const sendChallanNotification = async ({
  bulkOrder,
  customer,
  subject,
  emailSubject,
  ...
  const challanHTML = generateDeliveryChallanHTML(
    buildChallanData(bulkOrder, customer),
  );
  const pdfBuffer = await generatePDF(challanHTML);
  const orderNumber =
    bulkOrder.orders[0]?.orderNumber || bulkOrder._id.toString();
  ...
  if (!emailResult.success) {
    throw new Error(
      `Email delivery failed for challan to ${customer.businessEmail}`,
    );
  }
  ...
  if (customer.mobileNo1) {
    sendWhatsAppMedia({
      ...
      console.error("Challan WhatsApp media error:", err.message),
    );
  }
};
...
const sendInvoiceNotification = async ({ bulkOrder, customer }) => {
  ...
  if (!emailResult.success) {
    throw new Error(
      `Email delivery failed for invoice to ${customer.businessEmail}`,
    );
  }
  ...
  if (customer.mobileNo1) {
    sendWhatsAppMedia({
      ...
      console.error("Invoice WhatsApp media error:", err.message),
    );
  }
};

export const handleOrderBillingNotification = async ({
  bulkOrder,
  customer,
}) => {
  ...
  if (billingMode === "DC") {
    console.log(
      `[Billing] DC mode — challan will be sent on billing cycle for customer: ${customer._id}`,
    );
    return;
  }
};
...
export const sendDCBillingCycleChallan = async (customerId) => {
  try {
    ...
    if (!orders || orders.length === 0) {
      console.log(
        `No orders found for customer ${customerId} in billing cycle`,
      );
      return;
    }
