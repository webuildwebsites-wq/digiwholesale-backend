const VendorRxOrderTemplate = ({ vendorName, orderNumber, orderDate, customer, shipTo, items }) => {
  ...
  const itemRows = items.map((item, idx) => {
    ...
    <div style="border:1px solid #ddd;border-radius:6px;margin-bottom:20px;overflow:hidden;">
      <div style="background:#2980B9;color:#fff;padding:8px 14px;font-weight:bold;font-size:14px;">
...
<body style="margin:0;padding:0;background:#eeeeee;font-family:'Segoe UI',Roboto,Arial,sans-serif;">
  <div style="max-width:680px;margin:40px auto;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
    ...
    <div style="background:#2980B9;padding:20px 28px;color:#fff;">
    ...
    <div style="padding:24px 28px;background:#f0f9ff;border-bottom:2px solid #2980B9;">
    ...
    <div style="padding:24px 28px;">
      ...
      <table style="width:100%;border-collapse:collapse;margin-bottom:22px;background:#f9f9f9;border:1px solid #ddd;border-radius:6px;">
        <tr>
          <td style="padding:10px 14px;font-weight:bold;color:#2980B9;font-size:13px;text-transform:uppercase;white-space:nowrap;width:40%;">Order Number</td>
          <td style="padding:10px 14px;font-size:14px;">${fmt(orderNumber)}</td>
        </tr>
        <tr style="background:#fff;">
          <td style="padding:10px 14px;font-weight:bold;color:#2980B9;font-size:13px;text-transform:uppercase;">Order Date</td>
          <td style="padding:10px 14px;font-size:14px;">${fmt(orderDate)}</td>
        </tr>
        <tr>
          <td style="padding:10px 14px;font-weight:bold;color:#2980B9;font-size:13px;text-transform:uppercase;">Customer</td>
          <td style="padding:10px 14px;font-size:14px;">${fmt(customer.name)}</td>
        </tr>
        ${customer.phone ? `
        <tr style="background:#fff;">
          <td style="padding:10px 14px;font-weight:bold;color:#2980B9;font-size:13px;text-transform:uppercase;">Customer Phone</td>
          <td style="padding:10px 14px;font-size:14px;">${fmt(customer.phone)}</td>
        </tr>` : ""}
        ${shipTo ? `
        <tr>
          <td style="padding:10px 14px;font-weight:bold;color:#2980B9;font-size:13px;text-transform:uppercase;">Ship To</td>
          <td style="padding:10px 14px;font-size:14px;">${fmt(shipTo)}</td>
        </tr>` : ""}
      </table>

      <div style="font-size:16px;font-weight:700;color:#333;margin-bottom:14px;border-left:4px solid #2980B9;padding-left:10px;">
