    ...
    zone: {
      name: {
        ...
        required: function () {
          ...
          return (
            ["EMPLOYEE", "SUPERVISOR", "TEAMLEAD"].includes(
              this.EmployeeType,
            ) && dept === "SALES"
          );
        },
...
employee.pre("save", function () {
  try {
    if (this.isModified("isDeleted") && this.isDeleted === true) {
      ...
      console.log(
        `Employee ${this.employeeName} will be automatically deleted on ${expiryDate.toISOString()}`,
      );
    }
    ...
    if (this.isModified("isDeleted") && this.isDeleted === false) {
      this.expireAt = null;
      console.log(
        `Employee ${this.employeeName} restored - automatic deletion cancelled`,
      );
    }
  } catch (error) {
    console.log("Error : ", error);
...
employee.index(
  { expireAt: 1 },
  {
    ...
    partialFilterExpression: { expireAt: { $ne: null } },
  },
);
